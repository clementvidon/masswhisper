...
This runbook attaches the current `api_domain` to the already bootstrapped backend VM and enables TLS on the public API.
...
Create an `A` record pointing the current `api_domain` to the server IPv4 address.
...
dig +short A "$api_domain" @1.1.1.1 | grep -qx "$server_ip" && echo ok || echo fail

printf 'google A record matches server IPv4: '
dig +short A "$api_domain" @8.8.8.8 | grep -qx "$server_ip" && echo ok || echo fail
```
...
curl -s "http://$api_domain/.well-known/acme-challenge/ping" \
...
ssh "root@$server_ip" "
  ...
  certbot certonly --test-cert --non-interactive --agree-tos --no-eff-email -m "$my_email" \
    --webroot -w /var/www/certbot -d "$api_domain"
"

ssh "root@$server_ip" "
  printf 'staging certificate issued: '
  openssl x509 -in "/etc/letsencrypt/live/$api_domain/fullchain.pem" -noout -issuer \
...
ssh "root@$server_ip" "
  ...
  certbot certonly --non-interactive --agree-tos --no-eff-email -m "$my_email" \
    --webroot -w /var/www/certbot -d "$api_domain" --force-renewal
"
...
```bash
api_domain="$(terraform -chdir=infra/terraform output -raw api_domain)"
openssl s_client -connect "$api_domain:443" -servername "$api_domain" </dev/null 2>/dev/null \
  | openssl x509 -noout -subject -issuer -dates
```

Confirm that:

- the certificate subject contains the current `api_domain`
...
If the current `api_domain` cannot be pointed yet, use a temporary hostname under the same controlled DNS zone and repeat the same process with that name.

## State After This Runbook

- the current `api_domain` resolves to the backend VM IPv4
